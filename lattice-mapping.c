...
static int	llr_callback(char	*name, struct dg	*dg, struct tdl_line	*tdl, struct lattice_rule_set	*rs)
{
...
struct lattice_rule_set	*load_lattice_rules(char	*tdl_status)
{
	...
	if(process_tdl_dgs_status_r(tdl_status, (int (*)(char *, struct dg *, struct tdl_line *, void *))llr_callback, rs))
	{
...
struct token	*extract_token_from_dg(struct dg	*dg)
{
	...
	if(yy_mode)
	{
		struct dg	*iddlist_dg = dg_path(dg, token_id_path);
		struct dg	*idlist_dg = dg_hop(iddlist_dg, lookup_fname("LIST"));
		struct dg	*idlast_dg = dg_hop(iddlist_dg, lookup_fname("LAST"));
		while(idlist_dg != NULL && idlist_dg != idlast_dg)
		{
			struct dg	*idfirst_dg = dg_hop(idlist_dg, 1);	// FIRST
			if(idfirst_dg && idfirst_dg->type->name[0]=='"')
			{
				int id = atoi(idfirst_dg->type->name+1);
				idlist_dg = dg_hop(idlist_dg, 2);	// REST
				t->nids++;
				t->ids = slab_realloc(t->ids, sizeof(id)*(t->nids-1), sizeof(id)*t->nids);
				t->ids[t->nids-1] = id;
			}
			else break;
		}
...
int	lattice_mapping_rule_output(struct lattice	*lat, struct lattice_rule	*r, struct lattice_edge	**inputs, int	*input_ids, struct re_match_set	*mats, struct dg	**out_dg)
{
	...
	for(i=0;i<r->noutput;i++)
	{
		...
		if(mapping_tokens)
		{
			...
			if(yy_mode)
			{
				if(le[i].token->nids==1)
				{
					int j;
					for(j=0;j<r->ninput;j++)
						if(inputs[j]->token->nids == 1 && inputs[j]->token->ids[0] == le[i].token->ids[0])
						{
							le[i].token->oc = inputs[j]->token->oc;
						}
				}
			}
