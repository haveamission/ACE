...
static int profilercmp(const struct profiler	**a, const struct profiler	**b)
{
...
{
	qsort(prof->sub, prof->nsub, sizeof(struct profiler*), (int(*)(const void*,const void*))profilercmp);
}
