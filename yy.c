...
struct lattice_vertex	*parse_yy_get_vtx(int	id, struct lattice *tc)
{
...
int	parse_yy(char	*yy)
{
	...
	while(peek(&yy) == '(')
	{
		...
		*e->token = yy_read_token(&yy, &vfrom, &vto);
		...
		e->from = parse_yy_get_vtx(vfrom, tc);
		e->to = parse_yy_get_vtx(vto, tc);
