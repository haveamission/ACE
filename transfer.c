...
int tta_unify_vars(struct mrs_var	*vin, struct mrs_var	*vrule)
{
...
int	test_transfer_alignment(struct transfer_rule	*tr, struct mrs	*mrs, int	count, int	*align, int	total, int	hccount, int	*hcalignment, int	hctotal, int is_filter)
{
	...
	if(tta_unify_vars(mrs->ltop, tr->input.mrs->ltop))return -1;
	if(tta_unify_vars(mrs->ltop, tr->context.mrs->ltop))return -1;
	if(tta_unify_vars(mrs->index, tr->input.mrs->index))return -1;
	if(tta_unify_vars(mrs->index, tr->context.mrs->index))return -1;
	...
	for(i=0;i<hccount;i++)
	{
		...
		if(tta_unify_vars(hin.hi, hrule.hi))return -1;
		if(tta_unify_vars(hin.lo, hrule.lo))return -1;
	}
	allow_unify_swapping = 1;
...
int	atr0_filtercb(int	*filtalign, int *filthcalign, int *Did_filter)
{
	*Did_filter = 1;
	// return of 1 indicates we don't care about more alignments; one is enough to say it was filtered.
	return 1;
}

int	align_transfer_rule0(struct transfer_rule	*tr, struct mrs	*mrs, int	place, int	total, int	*alignment, int	hcplace, int	hctotal, int	*hcalignment, int	(*callback)(int	*alignment, int	*hcalignment, void	*ctx), int	is_filter, void *cbctx)
{
	int	i;
	if(place == total && hcplace == hctotal && !is_filter)
	{
		...
		if(tr->filter.neps || tr->filter.nhcons)
		{
			...
			forget_tmp();
			int	did_filter = 0;
			align_transfer_rule0(tr, mrs, place, tr->input.neps + tr->context.neps + tr->filter.neps, alignment, hcplace, tr->input.nhcons + tr->context.nhcons + tr->filter.nhcons, hcalignment, (int(*)(int*,int*,void*))atr0_filtercb, 1, &did_filter);
			forget_tmp();
			if(did_filter)
			{
		...
		int rv = callback(alignment, hcalignment, cbctx);
		forget_tmp();
		return rv;
	}
	...
	if(place < total)
	{
		...
		for(i=0;i<mrs->nrels;i++)
		{
			...
			DEBUG("passed test.\n");
			mrs->rels[i].active = 1;
			int rv = align_transfer_rule0(tr, mrs, place+1, total, alignment, hcplace, hctotal, hcalignment, callback, is_filter, cbctx);
			mrs->rels[i].active = 0;
			if(rv != 0)return rv;
		}
		return 0;
	...
	else
	{
		...
		for(i=0;i<mrs->nhcons;i++)
		{
			...
			int rv = align_transfer_rule0(tr, mrs, place, total, alignment, hcplace+1, hctotal, hcalignment, callback, is_filter, cbctx);
...
int	align_transfer_rule(struct transfer_rule	*tr, struct mrs	*mrs, int	(*callback)(int	*alignment, int	*hcalignment, void	*cbctx), void	*cbctx)
{
	int	i;
	for(i=0;i<mrs->nrels;i++)
		mrs->rels[i].active = 0;
	...
	return align_transfer_rule0(tr, mrs, 0, tr->input.neps + tr->context.neps, alignment, 0, tr->input.nhcons + tr->context.nhcons, hcalignment, callback, 0, cbctx);
}

struct trigger_found_alignment_ctx
{
	struct transfer_rule	*rule;
	void	(*trigger)(char	*lexid, struct dg	*cons, void	*ctx);
	void	*ctx;
	struct mrs	*mrsin;
	int	use_trigger_constraints;
};

static int	trigger_found_alignment(int	*alignment, int	*hcalignment, void *ctx)
{
	struct trigger_found_alignment_ctx	*myctx = ctx;
	struct dg	*flags = dg_hop(myctx->rule->dg, lookup_fname("FLAGS"));
	struct dg	*trig = dg_hop(flags, lookup_fname("TRIGGER"));
	struct dg	*trigcons = NULL;
	if(myctx->use_trigger_constraints)
	...
	{
		//printf("not triggering '%s' via '%s' because it is disabled for generation\n", lexid, myctx->rule->name);
		//return;
	}
	else
	{
		char	*aligned_on = "<none>";
		if(myctx->rule->input.neps + myctx->rule->context.neps > 0)
			aligned_on = myctx->mrsin->rels[alignment[0]].pred;
		//printf("triggering '%s' via '%s', aligned on %s\n", lexid, myctx->rule->name, aligned_on);
		if(trigcons)
		{
		...
		myctx->trigger(lexid, trigcons, myctx->ctx);
	}
...
void	activate_trigger_rules(struct mrs	*mrsin, void	trigger(char	*lexid, struct dg	*cons, void	*ctx), void	*ctx)
{
	int	i;
	...
	for(i=0;i<ntrigger_rules;i++)
	{
		struct trigger_found_alignment_ctx tfactx = {.trigger = trigger, .ctx = ctx, .rule = trigger_rules[i], .use_trigger_constraints = use_trigger_constraints, .mrsin = mrsin};
		align_transfer_rule(trigger_rules[i], mrsin, trigger_found_alignment, &tfactx);
	}
}

struct drotr_ctx
{
	struct transfer_rule	*R;
	struct mrs	*m;
};

int	drotr_found_alignment(int	*alignment, int	*hcalignment, void *ctx)
{
	struct drotr_ctx	*myctx = ctx;
	struct dg	*flags = dg_hop(myctx->R->dg, lookup_fname("FLAGS"));
	struct dg	*trig = dg_hop(flags, lookup_fname("TRIGGER"));
	char	lexid[256];
	sprintf(lexid, "%.*s", (int)strlen(trig->xtype->name)-2, trig->xtype->name+1);
	printf("rule %s matches EPs:", myctx->R->name);
	int j;
	for(j=0;j<myctx->R->input.neps + myctx->R->context.neps;j++)
		printf(" %s", myctx->m->rels[alignment[j]].pred);
	printf("    -> %s\n", lexid);
	if(temporary_expedient(lexid))
...
void debug_run_one_trigger_rule(struct mrs	*m, char	*rname, int detail)
{
	...
	struct drotr_ctx ctx = {.R = R, .m = m};
	align_transfer_rule(R, m, drotr_found_alignment, &ctx);
	debug_transfer = 0;
}
...
struct tfa_ctx
{
	struct transfer_rule	*tr;
	struct mrs				*min;
	struct transfer_mrs_set	*ts;
	int		pack_start;
	struct hash	*preds_seen;
	int	*Nalignments, *Nfires;
	int	first_route_only;
};

int	transfer_found_alignment(int	*alignment, int *hcalignment, void *ctx)
{
	struct tfa_ctx	*myctx = ctx;
	if(0 != process_one_transfer_result(myctx->tr, alignment, hcalignment, myctx->min, myctx->ts, myctx->pack_start, myctx->preds_seen))
		return 0;

	(*myctx->Nalignments)++;
	(*myctx->Nfires)++;
	return myctx->first_route_only;
}
...
int	transfer_mrs1(struct transfer_rule	*tr, struct transfer_mrs_set	*ts, int first_route_only, struct hash	*preds_seen)
{
	...
	for(i=0;i<ts->n && !cancel_task;i++)
	{
		...
		int	nalignments = 0;
		struct tfa_ctx ctx = {.tr = tr, .ts = ts, .first_route_only = first_route_only, .preds_seen = preds_seen, .pack_start = pack_start, .Nalignments = &nalignments, .Nfires = &nfires, .min = min};
		if(tr_quickcheck(tr, min))
			align_transfer_rule(tr, min, transfer_found_alignment, &ctx);
