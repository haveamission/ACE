...
//struct profiler	*instantiate_unify_profiler;
//struct profiler	*instantiate_copy_profiler;
...
void print_hypothesis(struct hypothesis *hyp, int (*print)(void*ctx, const char *fmt,...), void	*printctx, int	labelled);
int ctxprintf(void *ctx, char *fmt, ...);
...
struct decomposition *decompose_edge(struct edge	*edge, int *Ndecompositions)
{
	...
	if(n>100 && 0)
	{
		if(n > n2)printf("WARNING used more than n2 decomps\n");
		printf("[%d - %d] %s has %d decompositions ; allocated %d ; alt alloc size %d\n", edge->from, edge->to, edge->rule->name, n, nallocated, n2);
		printf("    daughters[0]->npack = %d	%s\n", edge->daughters[0]->npack, edge->daughters[0]->rule?edge->daughters[0]->rule->name:"norule");
		if(edge->rule->ndaughters>1)
			printf("    daughters[1]->npack = %d	%s\n", edge->daughters[1]->npack, edge->daughters[1]->rule?edge->daughters[1]->rule->name:"norule");
	}
...
again:
	...
	if(hyp->dg == (void*)1)
	{
		...
		//print_hypothesis(hyp, ctxprintf, NULL, 0); printf("\n");
		goto again;
	}
...
int instantiate_hypothesis(struct hypothesis *h)
{
	...
	if(unspecialize || !inhibit_pack)
	{
		...
		else
		{
			...
			//start_and_alloc_profiler(&instantiate_unify_profiler, "instantiate-unify", unpacking_profiler, NULL);
			for(i=0;i<arity;i++)
				if(unify_dg_tmp(h->rhslist[i]->dg, rdg, i))
				{
					...
					//unify_backtrace();
					//print_hypothesis(h, ctxprintf, NULL, 0); printf("\n\n");
					forget_tmp();
					h->dg = (void*)1;
			...
			//stop_profiler(instantiate_unify_profiler, (i<arity)?(i+1):arity);
			if(i<arity)
			{
			...
			//start_and_alloc_profiler(&instantiate_copy_profiler, "instantiate-copy", unpacking_profiler, NULL);
			if(g_mode == GENERATING)
			{
				int cant_share = 0;
				for(i=0;i<arity;i++)
			...
			//stop_profiler(instantiate_copy_profiler, 1);
			if(robustness_used)
			{
...
void	phf_print_tree0(struct hypothesis *hyp, int from, char	**genwordforms, struct phf_pt0_ctx	*ctx)
{
	int i;
	...
	if(e->lex && g_mode == GENERATING)
	{
		if(!genwordforms)
		{
			...
			phf_print_tree0(hyp, from, forms, ctx);
			return;
		}
	}
	...
	if(!ctx->labelled)
		ctx->print(ctx->printctx, " (%d %s%s%s %f %d %d", hyp->eid, sign, udx_at, udx_type,
						hyp->score, from, from+hyp->toklen);
	else ctx->print(ctx->printctx, " (\"%s\"", label_dag(hyp->dg, "?"));
	if(e->have)
	{
		for(i=0;i<e->have;i++)
		{
			phf_print_tree0(hyp->rhslist[i], from, genwordforms, ctx);
	...
	else if(e->lex)
	{
		...
		ctx->print(ctx->printctx, " (\"");
		for(i=0;i<ntokens;i++)
		{
			struct token	*tok = tokens[i];
			char	*form = tok->string;
			if(i>0)ctx->print(ctx->printctx, " ");
			ctx->print(ctx->printctx, "%s", slab_escape(form));
		}
		ctx->print(ctx->printctx, "\"");
		...
		if(enable_token_avms)
		{
			if(!ctx->labelled)
			{
				for(i=0;i<ntokens;i++)
				{
					...
					ctx->print(ctx->printctx, " %d \"%s\"", id, tokstr);
				}
			}
		}
		ctx->print(ctx->printctx, ")");
...
void print_hypothesis_from(struct hypothesis *hyp, int (*print)(void *ctx, const char *fmt,...), void *printctx, int	labelled, int	_from)
{
	static int timer = -1;
	...
	if(derivations_include_roots)
	{
		...
		if(enable_dublin && !root)
			root = "CSAW_ROBUST";
		assert(root != NULL);
		print(printctx, " (%s", root);
	}
	struct phf_pt0_ctx	ctx = {.labelled = labelled, .print = print, .printctx = printctx};
	phf_print_tree0(hyp, _from, NULL, &ctx);
	if(derivations_include_roots)print(printctx, ")");
	//set_slabstate(&st);
	stop_timer(timer, 1);
}
...
void print_hypothesis(struct hypothesis *hyp, int (*print)(void*ctx, const char *fmt,...), void	*printctx, int	labelled)
{
	print_hypothesis_from(hyp, print, printctx, labelled, 0);
}
...
int	dprintf_wctx(struct dprintf_ctx	*ctx, const char	*fmt, ...)
{
	va_list	v;
	va_start(v, fmt);
	int	l = vsnprintf(ctx->derivation+ctx->dlen, ctx->dalen-ctx->dlen, fmt, v);
	va_end(v);
	if(l >= ctx->dalen-ctx->dlen)
	{
		va_list v2;
		va_start(v2, fmt);
		ctx->dalen += l + 10240 ;
		ctx->derivation = realloc(ctx->derivation, ctx->dalen);
		l = vsnprintf(ctx->derivation+ctx->dlen, ctx->dalen-ctx->dlen, fmt, v2);
		va_end(v2);
	}
	ctx->dlen += l;
	return l;
...
{
	struct dprintf_ctx ctx = {.derivation = malloc(10240), .dalen = 10240, .dlen = 0};
	ctx.derivation[0] = 0;

	print_hypothesis_from(h, (int(*)(void*,const char*,...))dprintf_wctx, &ctx, 0, _from);
	return ctx.derivation;
}
...
char	*hypothesis_to_labelled_tree_string(struct hypothesis	*h)
{
	...
	print_hypothesis(h, (int(*)(void*,const char*,...))dprintf_wctx, &ctx, 1);
	return ctx.derivation;
}
...
int iterate_cell_root_hypotheses(struct chart_cell *cell, int (*process)(struct hypothesis *hyp, struct mrs *mrs, double	probability, void *ctx), int nbest, void *ctx)
{
	int	i, j;
		...
		if(0 == ires)
		{
			...
			if(is_root(hyp->dg))
			{
				...
				if(idiomatic || (g_mode == GENERATING))
				{
					...
					count += process(hyp, m, probability, ctx);
				}
			}
...
resume_after_oom:;
	if(!count)
	{
		...
		extern struct edge *dublin_root;
	...
	#define	LAST_DITCH_PROBABILITY	1E-10
				...
				process(last_ditch_h, m, LAST_DITCH_PROBABILITY, ctx);
				count++;
			}
